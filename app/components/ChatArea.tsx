...
import { Hash, Pin, Bell, Users, Search, Inbox, HelpCircle, PlusCircle, Smile, Gift, Sticker, Image as ImageIcon, Reply, X, CornerUpRight, MoreHorizontal, ThumbsUp, Heart, Laugh, Frown, Paperclip, Send, Edit2, Check, AtSign, Calendar, Video, Phone, Settings, MapPin, MessageSquare, Link as LinkIcon, Code, Bold, Italic, Strikethrough, List } from 'lucide-react';
...
const mockMessages: Message[] = [
  ...
  },
  {
    ...
    isCurrentUser: false,
    reactions: [
      { emoji: '🚀', count: 2, users: ['You', 'Sarah'] },
    ],
  },
];

// Mock tasks for mentions
...
export function ChatArea({ channelId, onTaskClick }: ChatAreaProps) {
  ...
  const handleForward = (msg: Message) => {
    // Mock forward functionality - in real app would show channel selector
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="h-10 px-3 flex items-center gap-3 border-b border-[#1e1f22] shadow-sm">
        ...
        <span className="text-gray-400 text-xs">Team collaboration and task updates</span>
        ...
        <div className="space-y-2">
          {messages.map((msg) => (
            ...
            >
              ...
              <div className={`absolute -top-3 ${msg.isCurrentUser ? 'left-3' : 'right-3'} opacity-0 group-hover:opacity-100 transition-opacity bg-[#1e1f22] rounded-md border border-[#3f4147] shadow-lg flex items-center gap-0.5 p-0.5 z-10`}>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              <Avatar className="h-8 w-8 mt-0.5 flex-shrink-0">
                <AvatarFallback className={msg.isCurrentUser ? 'bg-[#5865f2]' : ''}>
                  {msg.avatar}
                </AvatarFallback>
              </Avatar>
              <div className={`flex-1 min-w-0 ${msg.isCurrentUser ? 'flex flex-col items-end' : ''}`}>
                <div className={`flex items-baseline gap-2 ${msg.isCurrentUser ? 'flex-row-reverse' : ''}`}>
                  <span className={`text-sm ${msg.isCurrentUser ? 'text-[#5865f2]' : 'text-white'}`}>
                    {msg.author}
                  </span>
                  <span className="text-gray-400 text-[11px]">{msg.timestamp}</span>
                </div>
                
                {/* Reply indicator */}
                {msg.replyTo && (
                  <div className={`mt-0.5 mb-0.5 p-1.5 bg-[#2b2d31] rounded border-l-2 border-gray-500 ${
                    msg.isCurrentUser ? 'ml-auto max-w-md' : 'max-w-md'
                  }`}>
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <Reply size={10} className="text-gray-400" />
                      <span className="text-gray-400 text-[10px]">{msg.replyTo.author}</span>
                    </div>
                    <div className="text-gray-400 text-[11px] truncate">{msg.replyTo.content}</div>
                  </div>
                )}

                <div className={`text-gray-300 text-[13px] mt-0.5 ${
                  msg.isCurrentUser ? 'bg-[#5865f2] px-2.5 py-1.5 rounded-lg max-w-md' : ''
                }`}>
                  {msg.content}
                </div>
                ...
                {msg.task && (
                  ...
                  >
                    ...
                    <div className="text-white text-sm mb-0.5">{msg.task.title}</div>
                    <div className="text-gray-400 text-xs">{msg.task.description}</div>
                ...
                {msg.reactions && msg.reactions.length > 0 && (
                  <div className={`flex gap-1 mt-1 ${msg.isCurrentUser ? 'justify-end' : ''}`}>
                    {msg.reactions.map((reaction, index) => (
                      ...
                      >
                        ...
                        <span className={`text-[10px] ${reaction.users.includes('You') ? 'text-[#5865f2]' : 'text-gray-400'}`}>
                          {reaction.count}
                        </span>
                      </button>
                    ))}
      ...
      <div className="px-3 py-2 bg-[#313338] border-t border-[#1e1f22]">
        ...
        {replyingTo && (
          <div className="mb-2 p-1.5 bg-[#2b2d31] rounded-t-md border-l-2 border-[#5865f2] flex items-start justify-between">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 mb-0.5">
                <Reply size={10} className="text-gray-400" />
                <span className="text-gray-400 text-[10px]">Replying to {replyingTo.author}</span>
              </div>
              <div className="text-gray-300 text-xs truncate">{replyingTo.content}</div>
            </div>
            <Button
              variant="ghost"
              size="sm"
        ...
        <div className={`bg-[#383a40] ${replyingTo ? 'rounded-b-md' : 'rounded-md'}`}>
          <Input
