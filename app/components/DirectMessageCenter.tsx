...
const mockFriends: Friend[] = [
  ...
  { id: 'f5', name: 'Tom Wilson', email: 'tom@company.com', avatar: 'TW', status: 'dnd', customStatus: 'Do not disturb' },
  { id: 'f6', name: 'Lisa Anderson', email: 'lisa@company.com', avatar: 'LA', status: 'offline' },
  { id: 'f7', name: 'David Brown', email: 'david@company.com', avatar: 'DB', status: 'offline' },
  { id: 'f8', name: 'Rachel Green', email: 'rachel@company.com', avatar: 'RG', status: 'online', customStatus: 'Available' },
];
...
const mockDMs: DMConversation[] = [
  {
    ...
    lastMessage: 'Perfect! Take your time 👍',
    timestamp: '2m ago',
    unread: 2,
  },
  {
...
const mockFriendRequests: FriendRequest[] = [
  ...
  {
    id: 'fr2',
    from: { id: 'new2', name: 'Jane Doe', email: 'jane@company.com', avatar: 'JD', status: 'offline' },
...
export function DirectMessageCenter() {
  ...
  const [dms, setDms] = useState<DMConversation[]>(mockDMs);
  const [friendRequests, setFriendRequests] = useState<FriendRequest[]>(mockFriendRequests);
  const [newFriendInput, setNewFriendInput] = useState('');
  const [showAddFriendModal, setShowAddFriendModal] = useState(false);

  // Load last selected DM from localStorage
  useEffect(() => {
    const savedDMId = localStorage.getItem('lastSelectedDM');
    if (savedDMId) {
  ...
  // Save selected DM to localStorage whenever it changes
  useEffect(() => {
    if (selectedDM) {
  ...
  const filteredDMs = dms.filter(dm =>
    dm.friend.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <div className="text-gray-500 text-xs truncate">
                      {dm.isTyping ? (
                        <span className="text-[#5865f2]">typing...</span>
                      ) : (
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`text-sm transition-colors ${
                    friendsTab === 'online' ? 'text-white' : 'text-gray-400 hover:text-gray-200'
                  }`}
                >
                  Online
                </button>
                <button
                  ...
                  className={`text-sm transition-colors ${
                    friendsTab === 'all' ? 'text-white' : 'text-gray-400 hover:text-gray-200'
                  }`}
                >
                  All
                </button>
                <button
                  ...
                  className={`text-sm transition-colors flex items-center gap-1 ${
                    friendsTab === 'pending' ? 'text-white' : 'text-gray-400 hover:text-gray-200'
                  }`}
                >
                  Pending
                ...
                <button
                  ...
                  className={`text-sm transition-colors ${
                    friendsTab === 'blocked' ? 'text-white' : 'text-gray-400 hover:text-gray-200'
                  }`}
                >
                  Blocked
                </button>
              ...
              {(friendsTab === 'online' || friendsTab === 'all') && (
                <div className="p-6">
                  ...
                  <div className="space-y-2">
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-gray-400 text-sm">
                            {friend.customStatus || friend.email}
                          </div>
                        </div>
                    ...
                    {(friendsTab === 'online' ? onlineFriends : filteredFriends).length === 0 && (
                      <div className="text-center py-12">
                        <Users size={48} className="text-gray-600 mx-auto mb-4" />
                        <p className="text-gray-400">
                          {friendsTab === 'online' ? 'No friends online' : 'No friends found'}
                        </p>
                      </div>
                    )}
                  </div>
        ...
        {activeView === 'dms' && !selectedDM && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-gray-400 mb-6 max-w-md">
              Choose a conversation from the list on the left or start a new one by clicking on a friend.
            </p>
            <Button 
              onClick={() => {
                setActiveView('friends');
