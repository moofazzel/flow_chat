...
import { X, UserPlus, Search, Crown, Shield, Star, MoreVertical, AtSign, MessageSquare, Phone, Video, Mail, Ban, UserMinus, Settings as SettingsIcon } from 'lucide-react';
...
export function TeamMembersPanel({ isOpen, onClose }: TeamMembersPanelProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl h-[80vh] bg-[#313338] border-[#1e1f22] p-0" aria-describedby={undefined}>
        <DialogHeader className="px-6 py-4 border-b border-[#1e1f22]">
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-white text-xl">Team Members</DialogTitle>
        ...
        {showAddMember && (
          <div className="px-6 py-4 bg-[#2b2d31] border-b border-[#1e1f22]">
            <div className="flex items-end gap-3">
              ...
              <Button onClick={handleAddMember} className="bg-[#3ba55d] hover:bg-[#2d7d46] text-white">
                Add
              </Button>
              <Button
                onClick={() => {
        ...
        <ScrollArea className="flex-1 px-6 py-4">
          <Tabs defaultValue="all" className="w-full">
            <TabsList className="bg-[#1e1f22] border-[#3f4147] mb-4">
              ...
              <TabsTrigger value="online" className="data-[state=active]:bg-[#5865f2]">
                Online ({groupedMembers.online.length})
              </TabsTrigger>
              <TabsTrigger value="offline" className="data-[state=active]:bg-[#5865f2]">
                Offline ({groupedMembers.offline.length + groupedMembers.idle.length + groupedMembers.dnd.length})
              </TabsTrigger>
            </TabsList>
                ...
                {filteredMembers.map((member) => (
                  ...
                  >
                    <div className="relative">
                      ...
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-[#2b2d31] ${getStatusColor(member.status)}`}
                      />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {member.customStatus && (
                        <div className="text-gray-500 text-xs mt-0.5">{member.customStatus}</div>
                      )}
                    </div>
                ...
                {groupedMembers.online.map((member) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-gray-400 text-sm">{member.customStatus || 'Online'}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <TabsContent value="offline" className="mt-0">
              <div className="space-y-2">
                {[...groupedMembers.offline, ...groupedMembers.idle, ...groupedMembers.dnd].map((member) => (
                  <div
                    key={member.id}
                    className="flex items-center gap-3 p-3 bg-[#2b2d31] rounded-lg hover:bg-[#35363c] transition-colors"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-gray-500 text-sm">Last active: {member.lastActive}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {selectedMember && (
          <Dialog open={!!selectedMember} onOpenChange={() => setSelectedMember(null)}>
            <DialogContent className="bg-[#313338] border-[#1e1f22]">
              <DialogHeader>
                <DialogTitle className="text-white">Change Role for {selectedMember.name}</DialogTitle>
              ...
              <div className="space-y-4 py-4">
                <div>
                  <label className="text-gray-400 text-sm mb-2 block">Select Role</label>
                  <Select
                    value={selectedMember.role}
                    onValueChange={(value) => {
                      handleChangeRole(selectedMember.id, value as TeamMember['role']);
                      setSelectedMember(null);
                    }}
                  >
                    <SelectTrigger className="bg-[#1e1f22] border-[#3f4147] text-white">
                ...
                <div className="text-gray-400 text-sm space-y-2">
                  <p><strong className="text-white">Member:</strong> Can view and participate in channels</p>
                  <p><strong className="text-white">Moderator:</strong> Can manage messages and help moderate</p>
                  <p><strong className="text-white">Admin:</strong> Can manage members and server settings</p>
                </div>
              </div>
            </DialogContent>
          </Dialog>
